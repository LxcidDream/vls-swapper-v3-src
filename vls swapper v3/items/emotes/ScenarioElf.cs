...
{
    public partial class ScenarioElf : MaterialForm
    {
        ...
        public ScenarioElf()
        {
            ...
            this.Text = "Scenario";
            bool enabled = Settings.Default.ScenarioElfEnabled;
            if (enabled)
            {
                revert.Enabled = true;
        ...
        private static byte[] CMF1 = new byte[]
 {
        ...
        {
            if (Settings.Default.SmoothElfEnabled)
            {
                MetroFramework.MetroMessageBox.Show(this, "Smooth Moves[Take The Elf]" + actsomewhelse, error, MessageBoxButtons.OK, MessageBoxIcon.Error, 100);
                return;
            }
           ...
           CheckForIllegalCrossThreadCalls = false; int offsetskin1 = Settings.Default.offsetskin1; int offsetpick = Settings.Default.offsetpick; int offsetback = Settings.Default.offsetback; int offsetskin2 = Settings.Default.offsetskin2; int offsetemote = Settings.Default.offsetemote; int offsetlobby = Settings.Default.offsetlobby; int offsetpickmesh = Settings.Default.offsetpickmesh;
            ...
            foreach (long s in Researcher.FindPosition(fs1, 0, offsetemote, CMM))
            {
                ...
                binaryWrite.Close();
                Settings.Default.ScenarioElfEnabled = true;
                Settings.Default.Save();
                RichTextBoxInfo.Text = RichTextBoxInfo.Text + "\n[LOG] Dance 1/2 added!";
            }
